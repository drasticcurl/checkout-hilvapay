...
export function CardProducto({
  ...
  return (
    <div className="flex items-center gap-4 rounded-lg border border-borde p-4">
      {imagenUrl ? (
        ...
        <img src={imagenUrl} alt={nombre} className="h-16 w-16 shrink-0 rounded-md object-cover" />
      ) : (
        <div className="h-16 w-16 shrink-0 rounded-md bg-borde" aria-hidden="true" />
      )}
      <div className="min-w-0 flex-1">
        {subtitulo ? <p className="text-sm text-texto-suave">{subtitulo}</p> : null}
        <p className="font-semibold text-texto">{nombre}</p>
